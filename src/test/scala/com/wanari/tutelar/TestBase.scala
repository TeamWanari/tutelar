...
trait TestBase extends WordSpecLike with Matchers with MockitoSugar with BeforeAndAfterAll {
  ...
  abstract class ProviderTestScope[F[_]: MonadError[?[_], Throwable]]() {
    ...
    when(hookService.register(any[String], any[String], any[JsObject])).thenReturn(hookResponseRegister.pure[F])
    when(hookService.login(any[String], any[String], any[JsObject])).thenReturn(hookResponseLogin.pure[F])
