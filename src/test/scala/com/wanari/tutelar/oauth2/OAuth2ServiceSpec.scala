...
import com.wanari.tutelar.{AuthService, CsrfService, TestBase}
...
class OAuth2ServiceSpec extends TestBase {
  ...
  "TokenRequestHelper" should {
    "create jsonEntity" in useAS { implicit actorSystem =>
    ...
    "create formEntity" in useAS { implicit actorSystem =>
      implicit val materializer = ActorMaterializer()
      val trh                   = TokenRequestHelper("a", "b", "c", "d")
  ...
  "idAndRawReader" should {
    ...
    "parse out the given field (id)" in {
      ...
      val convertedJson = jsonStr.parseJson.convertTo[IdAndRaw](idAndRawReader("id"))

      convertedJson.id shouldBe "test"
      convertedJson.raw shouldBe jsonStr.parseJson
    }

    "parse out the given field (data)" in {
      val jsonStr       = """{"id": "test", "data":"sth"}"""
      val convertedJson = jsonStr.parseJson.convertTo[IdAndRaw](idAndRawReader("data"))
    ...
    "fails if the given id field is not in the json" in {
      val jsonStr       = """{"id": "test", "data":"sth"}"""
      val convertedJson = Try(jsonStr.parseJson.convertTo[IdAndRaw](idAndRawReader("sub")))
  ...
  "OAuth2Service" should {
    ...
    trait Scope {
      val service = new OAuth2Service[Try] {
        ...
        override def getApi(token: TokenResponseHelper) =
          if (token.access_token == "token") Try(IdAndRaw("id", JsString("raw"))) else ???
      }
    }
